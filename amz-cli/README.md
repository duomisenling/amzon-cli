...
**📖 完整命令使用手册:[docs/COMMANDS.md](docs/COMMANDS.md)**(39 条命令的示例、参数、注意事项,给运营同事和 Agent 看)

架构参照飞书官方 [lark-cli](https://github.com/larksuite/cli) 的三层命令与错误契约设计;完整规格见 `amazon-agent-cli-spec-v1.md`。
...
```powershell
cd D:\project_file\claude\cli\amz-cli
npm install
...
在 Cherry Studio 的项目 Agent 中，选择本仓库 `amz-cli` 作为工作目录即可自动发现 [`.claude/skills/amz-cli/SKILL.md`](.claude/skills/amz-cli/SKILL.md)；该入口会要求 Agent 读取正式 Skill，因此不需要手动粘贴系统提示词。Cherry 当前源码只识别 `.claude/skills/<技能目录>/SKILL.md` 这种目录结构。

发布到 GitHub 后，支持通用 Skills 安装器的 Agent 可按下面方式安装：

```powershell
npx skills add <GitHub组织或用户名>/<仓库名> -y -g
```

Cherry Studio 等不自动发现 Skills 的环境，仍可把 [`docs/AGENT.md`](docs/AGENT.md) 作为系统提示词参考。
...
- **买家 PII 一律不透出**:订单输出经白名单剥离(`src/shortcuts/orders/sanitize.ts`),地址/姓名/邮箱在 CLI 层就被丢弃
