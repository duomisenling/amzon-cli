...
`amz-cli` 是自带运行时的命令，直接调用即可，**不需要任何额外环境**。不要探测、检查或安装 Python、Node、pip、虚拟环境等运行时，也不要跑 `which python`、`python --version`、设置工作目录之类的环境准备命令——这些对本工具毫无必要，只会浪费时间并可能报错。只运行 `amz-cli ...` 命令本身；它找不到时就让用户重装，不要自行搭建或探测环境。
