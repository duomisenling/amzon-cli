...
- 正式安装使用编译版全局命令 `amz-cli ...`；仅源码开发者可在本仓库内使用 `node dist/cli.js ...`。
...
- Broker 模式的 Listing Seller ID 必须来自 Broker，`--seller-id` 不能兜底；Listing patch 的 `add`/`replace`/`merge` 必须带 `value`，且 `merge` 只用于官方支持的库存和可购买报价属性。
